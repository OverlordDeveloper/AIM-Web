    ...
    >
      ...
      >
        <h2 className={cn(
          ...
          enabled ? "text-sidebar-primary" : "text-sidebar-foreground/70"
        )}>{title}</h2>
        <div
          className={cn(
            "w-2.5 h-2.5 rounded-full transition-all",
            enabled ? "bg-sidebar-primary shadow-[0_0_8px_hsl(180_50%_45%/0.4)]" : "bg-sidebar-foreground/20"
          )}
        />
      </div>

      {enabled && children && (
        <div className="mt-3 space-y-2.5 border-t border-sidebar-border/50 pt-3" onClick={(e) => e.stopPropagation()}>
          {children}
        </div>
      )}
    </div>
